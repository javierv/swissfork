...
module Swissfork
    ...
    def allowed
      groups.each do |player_group|
        ...
        candidates.reject! do |downfloats|
          (downfloats & player_group).count < downfloats_in_this_group
        end
  ...
  private
    def number_of_downfloats_in(player_group)
      candidates.map { |downfloats| (downfloats & player_group).count }.max
    end
    ...
    def candidates
      @candidates ||= initial_candidates.select do |downfloats|
        (downfloats & players).count == number_of_downfloats
      end
    end
  end
end
